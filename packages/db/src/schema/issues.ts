...
export const issues = pgTable(
  "issues",
  {
    ...
    assigneeProfileRevision: bigint("assignee_profile_revision", { mode: "number" }).notNull().default(1),
  ...
  (table) => ({
    scopedIdentityUq: uniqueIndex("issues_scoped_identity_uq").on(table.id, table.companyId),

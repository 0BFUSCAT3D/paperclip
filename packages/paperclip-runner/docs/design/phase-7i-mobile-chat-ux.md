...
[Phase 4b interaction map](phase-4b-interaction-map.md) (chat console
baseline), [Phase 5 component decisions](phase-5-component-decisions.md).
Mockup: [`mockups/phase-7i-mobile-chat.html`](mockups/phase-7i-mobile-chat.html)
(annotated desktop + mobile renders in `knowledge/evidence/phase-07/`,
`phase-7i-*` slugs).

Phase 7I adds a **Codex-style interactive chat** over the same package-local
Phase 7 runner and mock `ControlPlanePort`. The board opens a scenario, chats
...
- Surface name in the UI: **Scenario chat** (header: “Phase 7 scenario chat —
  mock control plane”).
...
Same Vite entry, same app shell as 7F (`examples/phase7-explorer/`), new
...
2. **Model output** — left-aligned message cards on `--pcr-card` with the
   ...
   inherited verbatim from Phase 4b §1; reduced-motion renders instantly).
...
boundary proven in Phase 5, and no user-typed text is ever written to
...
- Streaming, flash cues, and auto-follow honor `prefers-reduced-motion`
  (instant/static variants, Phase 5 rule). Contrast from existing token
...
- **Driver seam:** Replay mode consumes recorded fixtures; Codex mode uses
  the bounded driver through the Phase 5 relay. The chat UI is
...
`phase-7i-<slug>-<viewport>.png` at **1440×900** and **390×844**, captured

# Phase 7 Interaction Map — Scenario Explorer and Screenshot Acceptance Spec
...
(annotated desktop + mobile renders in `knowledge/evidence/phase-07/`).

This map is the interaction contract for the Phase 7 browser scenario
...
- Product name in the UI: **Scenario Explorer** (header: “Phase 7 scenario
...
Package-local Vite entry `examples/phase7-explorer/` with script
`demo:phase7` (default port 4183; 4182 belongs to the Phase 6 demo). It runs
...
Desktop layout (≥ 64rem), one React tree (Phase 5 finding — never render two
trees):
...
(`Scenarios · Run · Inspect`), same pattern as the Phase 4b console. The
...
`spec/phase-07/eval-traceability.yaml` (all 106 cases, 16 groups) plus the
...
- **Mode control:** `Fake agent` (default) / `Codex (bounded)` segmented
  ...
  same server-side relay boundary proven in Phase 5.
...
pattern as Phase 5 `debugEvents`) — the semantic view is primary; raw
...
are inherited verbatim from the Phase 4b map §1 — do not redesign them.
...
Accessibility (WCAG POUR; inherits all Phase 4b/5 baseline rules):
...
scrollbar appears at 390px (Phase 5 finding).
...
| Missing/invalid fixture or artifact | Shell | Error card naming the artifact path and the regenerate command (`pnpm generate:phase7-inventory` / 7E command). No blank screen. |
...
Evidence lands in `knowledge/evidence/phase-07/` as
...
- No second app shell, no Tailwind/Radix, no dark mode, no markdown
  renderer, no command palette, no transcript virtualization — Phase 5
  rejections stand.

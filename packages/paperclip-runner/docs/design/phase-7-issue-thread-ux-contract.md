...
- A **segmented control** with exactly two segments — `Thread` and `Evidence` — sits under the
  header (pattern validated in the Phase 7I mobile review; a drawer was rejected there and
...
|-------|---------|--------|----------|
| `ready` | session attached, no active turn, issue non-terminal | normal input, `Send` primary | Send (Cmd/Ctrl+Enter), attach disabled in Phase 7 |
...
`knowledge/evidence/phase-07/ui/<slug>--<desktop|mobile>.png` (package-local). Every capture
...
pixel-identical (the Phase 7I byte-identical bar). Anything time-, random-, or
...
`knowledge/evidence/phase-07/cases/<group>/<case-id>--<viewport>.png`. The §10.2 matrix is

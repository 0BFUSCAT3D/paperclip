# Phase 7 Authorization and Exposure
...
Sources: `src/tools/phase7-tool-authorization.ts`,
`src/tools/phase7-semantic-tool-runtime.ts`, `src/tools/phase7-tool-bindings.ts`.
...
A denial is a `Phase7PolicyDenial`: `ok: false` with an `error.code` of
...
1. **Model-only delivery is separated from every observable boundary.**
   ...
   schema (`paperclip.phase7.model-tool-result.v1`) so it cannot be assigned to

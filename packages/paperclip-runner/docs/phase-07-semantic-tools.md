...
Sources: `src/tools/phase7-semantic-tool-catalog.ts`,
`src/tools/phase7-semantic-tool-types.ts`, `src/tools/phase7-tool-bindings.ts`,
...
[capability disposition](phase-07-capability-disposition.md) and
...
- `Phase7FakeAgentToolBinding` emits `{operationId, description, inputSchema,
  outputSchema}`.
- `Phase7CodexToolBinding` emits `{type: "function", name, description, strict:
  true, parameters}`.

Because both bindings derive from one array, the fake-agent and Codex operation
surfaces are byte-identical. The [conformance suite](phase-07-eval-conformance.md)

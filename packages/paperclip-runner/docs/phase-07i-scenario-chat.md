# Phase 7I Scenario Chat

The scenario chat is an interactive surface over the same package-local Phase 7
runner and [mock control plane](phase-07-mock-control-plane-port.md) that the
[scenario explorer](phase-07-scenario-explorer.md) reads. You pick a scenario,
...
Sources: `src/phase7/chat-session.ts`, `src/phase7/chat-script.ts`,
`src/phase7/scenario-execution.ts`, and `examples/phase7-explorer/`.
Interaction contract:
[Phase 7I mobile chat UX](design/phase-7i-mobile-chat-ux.md).
...
| Turn parity verdict | `phase7TurnParity` |
...
```sh
pnpm --filter @paperclipai/paperclip-runner run demo:phase7       # open the surface
pnpm --filter @paperclipai/paperclip-runner run test:phase7       # runtime + component tests
pnpm --filter @paperclipai/paperclip-runner run test:browser:phase7
pnpm --filter @paperclipai/paperclip-runner run record:phase7i    # §11 screenshot matrix
```

Walkthrough: [Phase 7I scenario chat tutorial](tutorials/phase-07i-scenario-chat.md).

# Phase 7 Eval-Derived Conformance

Phase 7E turns the Paperclip Evals corpus into an executable, offline
conformance suite. The suite derives its cases from the checked-in Phase 7
traceability derivative (`spec/phase-07/eval-traceability.yaml`) — it does
...
Sources: `src/conformance/phase7-eval-suite.ts` and its test
`src/conformance/phase7-eval-suite.test.ts`; the reporter
`scripts/run-phase7-eval-suite.mjs`.
...
- Each case declares an actor role, a task mode, an input scenario, and the
  ...
  [the capability contract](phase-07-capability-contract.md).
...
[verification commands reference](phase-07-verification-commands.md).

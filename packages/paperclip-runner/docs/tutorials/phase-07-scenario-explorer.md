# Phase 7 Clean-Start Tutorial: Capability Contract, Mock Control Plane, and Scenario Explorer
...
Phase 7 does not integrate the runner into Paperclip. It builds a package-local
...
read-only browser explorer over all of it. Real integration is Phase 8 and
requires separate approval. See
[the future binding boundary reference](../phase-07-future-binding-boundary.md).
...
```
Phase 7 eval conformance passed: 106 cases across 16 groups.
```

The report is written to
`knowledge/evidence/phase-07/eval-parity-report.{json,md}`. It is a
...
Open `http://127.0.0.1:4183/phase7-explorer/`. The explorer is read-only. It
...
`@paperclip-runner-local/phase7` alias — deliberately not the published package
name. See [the browser explorer reference](../phase-07-scenario-explorer.md).
...
- [Capability disposition](../phase-07-capability-disposition.md) — how every
...
- [Mock ControlPlanePort](../phase-07-mock-control-plane-port.md) — the
  deterministic in-memory adapter, its entity domains, and its boundary.
- [Semantic tool catalog](../phase-07-semantic-tools.md) — the
...
- [Eval conformance](../phase-07-eval-conformance.md) — how the 106 cases are
...
The exact results captured for the acceptance record are in the
[Phase 7 verification evidence](../../knowledge/evidence/2026-08-09-phase-07-verification.md).

## Known gaps and boundaries

- **The parity report breaks the OKF bundle if left in place.**
  `report:phase7-evals` writes `eval-parity-report.md` into the knowledge
  ...
  recorded for the Phase 7H QA gate.
- **Codex mode is disabled.** The explorer offers a bounded real-Codex binding
  sample only through `report:phase7-evals`; the browser holds no credential
...
- **No Paperclip integration.** Nothing here touches a real control plane,
  database, or provider. Phase 8 (ACPX) binds the real adapter behind the same
  seam and requires separate CTO approval.

## Where this sits

- Plan: Phase 7 plan, issue `PAP-16897` (see its `#document-plan` document).

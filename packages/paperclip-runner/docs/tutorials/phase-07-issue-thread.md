# Phase 7 Clean-Start Tutorial: The Paperclip-Style Issue Thread Over a Mock Control Plane
...
This is the canonical Phase 7 tutorial for the final build. It covers both agent
...
[execution modes and identity](../phase-07-execution-modes.md) for the rules that
separate them, and [the future binding boundary](../phase-07-future-binding-boundary.md)
for why none of this touches real Paperclip — that is Phase 8 and needs separate
approval.

For a deeper tour of the read-only scenario explorer and the capability
contract, see the companion
[scenario-explorer tutorial](phase-07-scenario-explorer.md); this page focuses
...
The report is written to `knowledge/evidence/phase-07/eval-parity-report.{json,md}`.
...
authorization and redaction rules, and the `Phase7IssueThreadSnapshot`
...
[issue-thread UX contract](../design/phase-7-issue-thread-ux-contract.md):
...
`knowledge/evidence/phase-07/ui/`. Every mobile capture asserts no horizontal
...
The frames land in `knowledge/evidence/phase-07/ui-live/`. They are intentionally
...
```sh
pnpm --filter @paperclipai/paperclip-runner trace:phase7 -- --json
```
...
- No ACPX implementation and no real control-plane integration. Real Paperclip
  binding is Phase 8 and requires separate approval.
...
References: [issue-thread UI](../phase-07-issue-thread-ui.md),
[live runnerd and Codex loop](../phase-07-live-runnerd-codex.md),
[execution modes and identity](../phase-07-execution-modes.md),
[final evidence manifest](../../knowledge/evidence/2026-08-10-phase-07i-final-evidence.md).

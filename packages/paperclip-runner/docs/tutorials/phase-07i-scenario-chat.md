# Phase 7I Tutorial: Chat With the Mock Control Plane

**Time to first success: about 3 minutes.** One command opens a chat where you
send prompts to a Phase 7 scenario and watch exactly how the mock Paperclip
...
Phase 7I extends the Phase 7 browser surface; it does not integrate the runner
into Paperclip. Real integration is Phase 8 and requires separate approval. See
...
```sh
pnpm --filter @paperclipai/paperclip-runner run demo:phase7
```

Open <http://127.0.0.1:4183/phase7-explorer/#/chat/ap-mcp-gate-01>.
...
Open <http://127.0.0.1:4183/phase7-explorer/#/chat/ix-checkbox-01?replay=fake>.
...
```sh
pnpm --filter @paperclipai/paperclip-runner run test:phase7
pnpm --filter @paperclipai/paperclip-runner run test:browser:phase7
pnpm --filter @paperclipai/paperclip-runner run record:phase7i
```
...
ten acceptance screenshots to `knowledge/evidence/phase-07/` — failing if any
...
Reference: [Phase 7I scenario chat](../phase-07i-scenario-chat.md).

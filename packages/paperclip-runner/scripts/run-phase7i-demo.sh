...
: "${PHASE7I_PUBLIC_ORIGIN:?PHASE7I_PUBLIC_ORIGIN is required}"
: "${PHASE7I_COMMIT_SHA:?PHASE7I_COMMIT_SHA is required}"
: "${PHASE7I_PROXY_PORT:?PHASE7I_PROXY_PORT is required}"

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
package_root="$(cd "$script_dir/.." && pwd)"
workspace_root="$(cd "$package_root/../.." && pwd)"
socket_dir="${PHASE7I_SOCKET_DIR:-/tmp/paperclip-phase7i-pap-16917}"
socket_path="$socket_dir/phase7i.sock"
...
unshare -Un \
  ...
  env -i \
    ...
    PHASE7I_BASE_PATH="${PHASE7I_BASE_PATH:-}" \
    PHASE7I_COMMIT_SHA="$PHASE7I_COMMIT_SHA" \
    PHASE7I_ALLOW_INSECURE_HTTP="${PHASE7I_ALLOW_INSECURE_HTTP:-}" \
    PHASE7I_SOCKET_PATH="$socket_path" \
  node \
    --max-old-space-size=256 \
    --permission \
    --allow-fs-read="$package_root/dist" \
    --allow-fs-read="$package_root/dist-phase7" \
    --allow-fs-read="$package_root/node_modules" \
    --allow-fs-read="$package_root/package.json" \
    --allow-fs-read="$package_root/spec/phase-07/eval-traceability.yaml" \
    --allow-fs-read="$workspace_root/node_modules" \
    --allow-fs-write="$socket_dir" \
    "$package_root/dist/phase7/phase7i-demo-server.js" &
server_pid=$!
...
env -i \
  ...
  PHASE7I_PROXY_HOST="${PHASE7I_PROXY_HOST:-127.0.0.1}" \
  PHASE7I_ALLOW_INSECURE_HTTP="${PHASE7I_ALLOW_INSECURE_HTTP:-}" \
  PHASE7I_SOCKET_PATH="$socket_path" \
node \

...
const readInventory = async (name) => decodeInventory(await readFile(resolve(packageRoot, `spec/phase-07/${name}.yaml`), "utf8"));
const inventories = {
...
const expected = {
  [resolve(packageRoot, "src/generated/phase7-capability-contract.ts")]: renderContractModule(inventories),
  [resolve(packageRoot, "docs/phase-07-capability-contract.md")]: renderDocumentation(inventories),
};
...
if (errors.length > 0) {
  process.stderr.write(`Phase 7 inventory check failed:\n${errors.map((error) => `- ${error}`).join("\n")}\n`);
  process.exitCode = 1;
} else {
  process.stdout.write("Phase 7 inventory completeness and generated-output checks passed.\n");
}

#!/usr/bin/env node
/**
 * Live smoke for the Phase 7M clean-room chat.
 *
 ...
 *
 * Usage: node scripts/phase7-clean-room-smoke.mjs [--json]
 */
...
async function main() {
  ...
  let summary = {};
  try {
    const opened = await get("/api/phase7/ui/cleanroom/session");
    ...
    const fresh = await post("/api/phase7/ui/cleanroom/session", { sessionId: opened.sessionId });
    ...
    if (!asJson) process.stdout.write("Phase 7 clean-room live smoke passed.\n");
  } finally {

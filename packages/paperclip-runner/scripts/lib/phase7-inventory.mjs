...
const sourceHeader = "# GENERATED FILE — DO NOT EDIT. Run pnpm generate:phase7-inventory.\n";
...
export function validateInventories(inventories) {
  ...
  for (const group of phase7Groups) if (!groups.has(group)) errors.push(`evaluations is missing group ${group}.`);
  if (groups.size !== phase7Groups.length) errors.push(`evaluations has unexpected groups: ${[...groups].sort().join(", ")}.`);
...
export function renderContractModule(inventories) {
  ...
  return `// Generated by scripts/generate-phase7-inventory.mjs. Do not edit.\n\nexport type Phase7PrimaryDisposition =\n  | "control_plane_owned"\n  | "always_agent_tool"\n  | "optional_agent_tool";\n\nexport const phase7InventoryCounts = ${JSON.stringify(counts, null, 2)} as const;\n\nexport const phase7EvalGroups = ${JSON.stringify(phase7Groups, null, 2)} as const;\n\nexport const phase7InventoryContract = {\n  schemaVersion: 2,\n  normativeInventories: [\n    "spec/phase-07/capabilities.yaml",\n    "spec/phase-07/eval-traceability.yaml",\n  ],\n  legacyAliasIndexes: ["spec/phase-07/mcp-tool-map.yaml"],\n  primaryDispositions: [\n    "control_plane_owned",\n    "always_agent_tool",\n    "optional_agent_tool",\n  ] as const,\n} as const;\n`;
}

export function renderDocumentation(inventories) {
  const groupCounts = Object.fromEntries(phase7Groups.map((group) => [group, inventories.evaluations.rows.filter((row) => row.group === group).length]));

...
import { phase7Groups, validateInventorySchema, validateInventories } from "./lib/phase7-inventory.mjs";

const inventorySchema = JSON.parse(await readFile(
  resolve(import.meta.dirname, "../spec/phase-07/inventory.schema.json"),
  "utf8",
));
...
function validInventories() {
  const evaluations = Array.from({ length: 106 }, (_, index) => row(`eval-${index}`, phase7Groups[index % phase7Groups.length]));
...
test("phase 7 inventory validator accepts exact baseline counts", () => {
...
test("phase 7 inventory validator rejects duplicate rows and invalid dispositions", () => {
  const inventories = validInventories();
...
test("phase 7 inventory validator rejects an independent MCP classification", () => {
  const inventories = validInventories();
...
test("phase 7 inventory validator rejects missing, duplicate, and unknown MCP folds", () => {
  const inventories = validInventories();

...
/**
 * Records the Phase 7I screenshot acceptance matrix.
 *
...
const evidenceDirectory = resolve(packageRoot, "knowledge/evidence/phase-07");
...
async function freePort() {
  const explicit = process.env.PHASE7_EVIDENCE_PORT;
...
async function main() {
  ...
  const overflows = [];
  try {
    await waitForServer(`${origin}/phase7-explorer/`, 60_000);
    ...
    try {
      for (const viewport of VIEWPORTS) {
        ...
        for (const shot of PHASE7I_SHOTS) {
          ...
          const path = resolve(evidenceDirectory, `phase-7i-${shot.slug}-${viewport.label}.png`);
          await page.screenshot({ path, fullPage: true });
          captured.push(`phase-7i-${shot.slug}-${viewport.label}.png`);
  ...
  await writeFile(
    resolve(evidenceDirectory, "phase-7i-screenshot-manifest.json"),
    `${JSON.stringify(
      {
        schema: "paperclip.phase7.screenshot-manifest.v1",
        source: "docs/design/phase-7i-mobile-chat-ux.md §11",
  ...
  process.stdout.write(`Recorded ${captured.length} Phase 7I chat screenshots.\n`);
}

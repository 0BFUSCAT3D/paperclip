...
import {
  ...
  type Phase6StandaloneDemoResult,
...
function App() {
  ...
  const [result, setResult] = React.useState<Phase6StandaloneDemoResult | null>(null);
  const [running, setRunning] = React.useState(false);

  const run = React.useCallback(async () => {
    setRunning(true);
    try {
      setResult(await runPhase6StandaloneDemo({ featureFlagEnabled, killSwitchEnabled }));
    } finally {
      setRunning(false);
    }
  ...
  return (
    <main>
      <header>
        ...
        <h1>Phase 6 standalone adapter demo</h1>
...
if (root === null) throw new Error("Phase 6 standalone demo root is missing.");

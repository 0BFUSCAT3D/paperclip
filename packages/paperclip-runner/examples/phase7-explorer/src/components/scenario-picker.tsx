...
import type { Phase7ScenarioIndexEntry } from "@paperclip-runner-local/phase7";

import { DISPOSITION_LABEL, groupLabel, PARITY_LABEL, ROLE_LABEL } from "../labels.js";
import { activeFilterChips, hasActiveFilters, type Phase7Filters } from "../route.js";
...
export interface Phase7FacetValue {
  value: string;
  label: string;
  count: number;
}

export interface Phase7PickerFacets {
  group: Phase7FacetValue[];
  disposition: Phase7FacetValue[];
  role: Phase7FacetValue[];
  claim: Phase7FacetValue[];
  parity: Phase7FacetValue[];
}

export interface ScenarioPickerProps {
  entries: readonly Phase7ScenarioIndexEntry[];
  visible: readonly Phase7ScenarioIndexEntry[];
  facets: Phase7PickerFacets;
        ...
        <div className="pcr7-chip-row" data-testid="active-filters">
          {chips.map((chip) => (
            <button
              ...
              onClick={() =>
                onFilterChange(chip.key === "query" ? { query: "" } : ({ [chip.key]: null } as Partial<Phase7Filters>))
              }
            >
            ...
            {facets[key].map((facet) => {
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    onFilterChange({ [key]: active ? null : facet.value } as Partial<Phase7Filters>)
                  }
                >
...
function groupEntries(
  entries: readonly Phase7ScenarioIndexEntry[],
): Array<[string, Phase7ScenarioIndexEntry[]]> {
  const byGroup = new Map<string, Phase7ScenarioIndexEntry[]>();
...
export function buildFacets(
  entries: readonly Phase7ScenarioIndexEntry[],
  countFor: (key: keyof Phase7PickerFacets, value: string) => number,
): Phase7PickerFacets {

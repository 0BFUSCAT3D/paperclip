...
import {
  PHASE7_ACTIVITY_FILTERS,
  type Phase7ActivityFilter,
} from "../route.js";

/**
 * Activity stream (7I map §4).
 *
...
const FILTER_LABEL: Record<Phase7ActivityFilter, string> = {
  all: "All",
  tools: "Tools",
...
const SECTION_FILTER: Record<SectionName, Phase7ActivityFilter> = {
  exposure: "tools",
  calls: "tools",
  authorization: "authorization",
  control: "control",
  ...
  return (
    <div className="pcr7-activity" data-testid="activity-stream">
      <div className="pcr7-activity-filters" role="group" aria-label="Activity filter">
        {PHASE7_ACTIVITY_FILTERS.map((value) => (
          <button
            key={value}
            type="button"
            className="pcr7-chip"
 ...
 * the one that did.
 */
function TurnDiff({ turn }: { turn: Phase7ChatTurn }) {
...
export function activityDenialCount(artifact: Phase7ChatSessionArtifact): number {

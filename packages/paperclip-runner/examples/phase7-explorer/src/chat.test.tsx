...
import { parsePhase7Route, serializePhase7Route } from "./route.js";

/**
 * Phase 7I chat rendering contract.
 *
...
const sessions = new Map<string, Phase7ChatSessionArtifact>();

beforeAll(async () => {
  index = buildPhase7ScenarioIndex(await readFile(MANIFEST_PATH, "utf8"));
  for (const id of ["ix-checkbox-01", "ap-mcp-gate-01"]) {
    sessions.set(id, await phase7ReplayChatSession(entryFor(id)));
  }
});
...
function sessionFor(id: string): Phase7ChatSessionArtifact {
...
describe("chat routes and shell", () => {
  ...
  it("keeps the explorer routes untouched", () => {
    const route = parsePhase7Route("#/case/ap-mcp-gate-01?run=fake&view=authorization");
    expect(route.surface).toBe("explorer");
    expect(route.caseId).toBe("ap-mcp-gate-01");
    expect(serializePhase7Route(route)).toBe("#/case/ap-mcp-gate-01?run=fake&view=authorization");
  });

  it("renders three landmarks and the chat segments on a chat route", () => {
    const markup = renderToStaticMarkup(
      <ExplorerApp index={index} initialRoute={parsePhase7Route("#/chat/ix-checkbox-01")} />,
    );
  ...
  it("names an unknown chat scenario instead of falling back to home", () => {
    const markup = renderToStaticMarkup(
      <ExplorerApp index={index} initialRoute={parsePhase7Route("#/chat/not-a-real-case")} />,
    );

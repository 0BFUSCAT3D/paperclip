...
import { phase7ChatScript } from "./chat-script.js";
import { phase7ScenarioFixture } from "./scenario-fixtures.js";
import { Phase7ChatSession, phase7ReplayChatSession } from "./chat-session.js";
import type {
  Phase7ScenarioIndex,
...
/**
 * Phase 7I chat session contract.
 *
...
describe("phase 7I chat session", () => {
  it("seeds turn 0 with control-plane work and no agent tool", async () => {
    const session = await Phase7ChatSession.open(entryFor("ix-checkbox-01"));
    try {
  ...
  it("numbers turns, records the user message, and scopes evidence per turn", async () => {
    const artifact = await phase7ReplayChatSession(entryFor("ix-checkbox-01"));
  ...
  it("keeps a denial inside its own turn with its own authorization slice", async () => {
    const artifact = await phase7ReplayChatSession(entryFor("ap-mcp-gate-01"));
  ...
  it("accumulates state across turns and reproduces it exactly on replay", async () => {
    const first = await phase7ReplayChatSession(entryFor("ap-mcp-gate-01"));
    const second = await phase7ReplayChatSession(entryFor("ap-mcp-gate-01"));
  ...
  it("does not leak state from a closed session into a fresh one", async () => {
    const used = await Phase7ChatSession.open(entryFor("ix-checkbox-01"));
    ...
    const fresh = await Phase7ChatSession.open(entryFor("ix-checkbox-01"));
    try {
  ...
  it("never lets a raw secret reach a chat artifact", async () => {
    const artifact = await phase7ReplayChatSession(entryFor("rs-secret-hygiene-01"));
  ...
  it("replays every corpus scenario in chat without a harness failure", async () => {
    for (const entry of index.entries) {
      const artifact = await phase7ReplayChatSession(entry);

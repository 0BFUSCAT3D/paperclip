...
import type { Phase7ScenarioFixture } from "./scenario-fixtures.js";
import {
...
/**
 * Conversational scripts for the Phase 7I chat.
 *
 ...
 */
export interface Phase7ControlPlaneCommandStep {
...
export type Phase7ChatStep = Phase7PlanStep | Phase7ControlPlaneCommandStep;

export interface Phase7ChatScriptTurn {
  /** The prompt the scripted board member sends to open this turn. */
  prompt: string;
  steps: Phase7ChatStep[];
}

export interface Phase7ChatScript {
  scenarioId: string;
  /** True when the turns were authored for chat rather than derived. */
  authored: boolean;
  turns: Phase7ChatScriptTurn[];
  controlPlaneCapabilities: Array<{ operationId: string; reason: string }>;
  restraint: boolean;
}

export function phase7ChatScript(
...
type ChatScriptFactory = (entry: Phase7ScenarioIndexEntry) => Phase7ChatScriptTurn[];
 ...
 */
function derivedPrompt(entry: Phase7ScenarioIndexEntry): string {
  return `Handle this wake: ${entry.title}`;
}

function message(text: string): Phase7ChatStep {
  return { kind: "agent_message", text };
}

function read(operationId: string, input: Phase7JsonValue, summary: string): Phase7ChatStep {

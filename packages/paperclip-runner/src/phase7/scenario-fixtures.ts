...
import type { Phase7ScenarioIndexEntry } from "./scenario-explorer-types.js";
import type { Phase7SeedKind } from "./scenario-index.js";

/**
 * Deterministic fixture seeds for the Phase 7 scenario explorer.
 *
...
function task(overrides: Partial<Phase7FixtureTask> & { id: string; identifier: string; title: string }): Phase7FixtureTask {
  return {
...
export interface Phase7ScenarioFixture {
...
export function phase7ScenarioFixture(entry: Phase7ScenarioIndexEntry): Phase7ScenarioFixture {
  const seedKind = entry.seedKind as Phase7SeedKind;
  const activeTask = task({
    id: TASK_ID,
    identifier: "MCK-102",
    title: entry.title,
    description: `Phase 7 fixture task for eval case ${entry.id}.`,
    status: "todo",
  ...
  const refs: Phase7ScenarioFixture["refs"] = {
    approvalId: null,
    documentKey: null,
    baseRevisionId: null,
    interactionId: null,
...
function applySeedKind(
  seedKind: Phase7SeedKind,
  seed: Phase7FixtureSeed,
  refs: Phase7ScenarioFixture["refs"],
...
function addPeerActor(seed: Phase7FixtureSeed): void {
...
export const PHASE7_FIXTURE_EPOCH_MS = FIXTURE_EPOCH_MS;
export const PHASE7_FIXTURE_TASK_ID = TASK_ID;
export const PHASE7_FIXTURE_ACTOR_ID = ACTOR_ID;
export const PHASE7_FIXTURE_PEER_ACTOR_ID = PEER_ACTOR_ID;

...
import { PHASE7_CONTROL_PLANE_OWNED_OPERATION_IDS } from "../tools/phase7-semantic-tool-types.js";
import {
...
import { phase7EvalSuiteLookup } from "./scenario-parity.js";
import { phase7RunScenario } from "./scenario-runner.js";
import type { Phase7ScenarioIndex, Phase7ScenarioIndexEntry } from "./scenario-explorer-types.js";
...
let index: Phase7ScenarioIndex;

beforeAll(async () => {
  index = buildPhase7ScenarioIndex(await readFile(MANIFEST_PATH, "utf8"));
});

function entryFor(id: string): Phase7ScenarioIndexEntry {
  const entry = phase7ScenarioEntry(index, id);
  expect(entry, `scenario ${id} is missing from the index`).toBeDefined();
  return entry!;
}

describe("phase 7 scenario index", () => {
  ...
  it("declares a profile for every fixture the manifest names", () => {
    const declared = new Set(phase7ScenarioProfileCoverage());
  ...
  it("is deterministic across two builds", async () => {
    ...
    expect(JSON.stringify(buildPhase7ScenarioIndex(source))).toBe(
      JSON.stringify(buildPhase7ScenarioIndex(source)),
    );
  });
});

describe("phase 7 scenario plans", () => {
  it("plans only operations the semantic catalog defines", () => {
    const catalog = new Set(PHASE7_SEMANTIC_TOOL_CATALOG.map((tool) => tool.operationId));
    for (const entry of index.entries) {
      const plan = phase7ScenarioPlan(entry, phase7ScenarioFixture(entry));
  ...
  it("never plans a control-plane-owned capability as an agent tool", () => {
    const controlPlane = new Set<string>(PHASE7_CONTROL_PLANE_OWNED_OPERATION_IDS);
    for (const entry of index.entries) {
      const plan = phase7ScenarioPlan(entry, phase7ScenarioFixture(entry));
      for (const step of plan.steps) {
        if (step.kind !== "tool_call") continue;
...
describe("phase 7 scenario runs", () => {
  ...
  it("passes parity for every scenario", async () => {
    const artifacts = await Promise.all(index.entries.map((entry) => phase7RunScenario(entry)));
  ...
  it("uses fixture time only — no wall-clock instant appears in a fake run", async () => {
    const artifact = await phase7RunScenario(entryFor("hb-scoped-wake-01"));
  ...
  it("shows optional tools with the grant that unlocked them", async () => {
    const artifact = await phase7RunScenario(entryFor("rf-api-mgr-heartbeat-01"));
  ...
  it("renders a typed denial without exposing protected state", async () => {
    const artifact = await phase7RunScenario(entryFor("ap-mcp-gate-01"));
  ...
  it("never lets a raw secret reach the run artifact", async () => {
    const artifact = await phase7RunScenario(entryFor("rs-secret-hygiene-01"));
  ...
  it("keeps restraint legible when the correct behaviour is absence", async () => {
    const artifact = await phase7RunScenario(entryFor("rs-question-only-01"));
  ...
  it("registers the deliverable before the task is finished", async () => {
    const artifact = await phase7RunScenario(entryFor("ar-upload-before-done-01"));
  ...
  it("carries the Phase 7E verdict through without re-judging it", async () => {
    ...
    const withReport = await phase7RunScenario(entry, { evalSuite: lookup });

...
import { buildPhase7ScenarioIndex } from "./scenario-index.js";
...
function assertArtifactBounds(artifact: Phase7ChatSessionArtifact): void {
...
export function forbiddenPhase7iEnvironmentNames(env: NodeJS.ProcessEnv): string[] {
  return Object.keys(env).filter((name) => FORBIDDEN_ENV_NAME.test(name)).sort();
}

export function assertPhase7iEnvironmentSafe(env: NodeJS.ProcessEnv): void {
  const forbidden = forbiddenPhase7iEnvironmentNames(env);
  if (forbidden.length > 0) {
    throw new Error(`Phase 7I refused forbidden environment names: ${forbidden.join(", ")}`);
  }
}

export class Phase7iDemoRuntime {
  ...
  private scenarioIndex: Phase7ScenarioIndex | null = null;
  private ready = false;
  ...
  constructor(options: RuntimeOptions) {
    ...
    this.staticRoot = resolve(this.packageRoot, "dist-phase7");
  ...
  async initialize(): Promise<void> {
    const manifest = await readFile(
      resolve(this.packageRoot, "spec/phase-07/eval-traceability.yaml"),
      "utf8",
    );
    this.scenarioIndex = buildPhase7ScenarioIndex(manifest);
    const probe = this.scenarioIndex.entries[0];
    if (!probe) throw new Error("Phase 7 scenario corpus is empty");
  ...
  private async openConcreteMockSession(entry: Phase7ScenarioIndexEntry): Promise<Phase7ChatSession> {
    ...
    return await Phase7ChatSession.openWithMockAdapter(entry, adapter, { mode: "fake" });
  }
  ...
  private async serveStatic(req: IncomingMessage, res: ServerResponse, pathname: string): Promise<void> {
    ...
    const relativePath = decoded === "/" ? "phase7-explorer/index.html" : decoded.replace(/^\/+/, "");
    ...
    if (relativePath === "phase7-explorer/index.html") {
      const html = body.toString("utf8").replace(
        "</head>",
        '<meta name="phase7i-api-base" content="api/phase7" /></head>',
      );
  ...
  private async handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    try {
      ...
      const url = new URL(req.url ?? "/", "http://phase7i.invalid");
      const pathname = this.stripBasePath(url.pathname);
      if (pathname === "/api/phase7/health" && req.method === "GET") {
        route = "health";
        this.assertProxyAdmission(req);
...
async function main(): Promise<void> {
  assertPhase7iEnvironmentSafe(process.env);
  const socketPath = process.env.PHASE7I_SOCKET_PATH?.trim();
  const publicOrigin = process.env.PHASE7I_PUBLIC_ORIGIN?.trim();
  if (!socketPath || !isAbsolute(socketPath)) throw new Error("PHASE7I_SOCKET_PATH must be absolute");
  if (!publicOrigin) throw new Error("PHASE7I_PUBLIC_ORIGIN is required");

...
export type Phase7TailnetGatewayConfig = {
  bindHost: string;
  listenPort: number;
...
export type Phase7TailnetAdmissionFailure = {
  status: number;
  code: string;
  message: string;
};
 ...
 * `fetch()` request.
 */
export function phase7TailnetFetchMetadataAllowed(headers: IncomingHttpHeaders): boolean {
  const expectations = [
...
export function phase7TailnetAdmissionFailure(
  input: AdmissionInput,
): Phase7TailnetAdmissionFailure | null {
  ...
  const isApi = input.pathname.startsWith("/api/phase7/ui/");
  ...
  if (!phase7TailnetFetchMetadataAllowed(input.headers)) {
    return {
      status: 403,
...
function parsePort(value: string | undefined): number {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65_535) {
    throw new Error("Phase 7 tailnet gateway requires valid listener and upstream ports");
  }
  return port;
}

export function resolvePhase7TailnetGatewayConfig(
  env: NodeJS.ProcessEnv,
): Phase7TailnetGatewayConfig {
  const bindHost = env.PHASE7_GATEWAY_BIND_HOST ?? "";
  const upstreamHost = env.PHASE7_UPSTREAM_HOST ?? "127.0.0.1";
  const publicOrigin = new URL(env.PHASE7_PUBLIC_ORIGIN ?? "");
  ...
  if (
    ...
    throw new Error("Phase 7 tailnet gateway requires an exact bind, loopback upstream, and public origin");
  }

  return {
    bindHost,
    listenPort: parsePort(env.PHASE7_GATEWAY_PORT),
    upstreamHost,
    upstreamPort: parsePort(env.PHASE7_UPSTREAM_PORT),
    publicOrigin,
    buildIdentifier: env.PHASE7_BUILD_IDENTIFIER ?? "unknown",
    sourceCommit: env.PHASE7_SOURCE_COMMIT ?? "unknown",
    bundleSha256: env.PHASE7_BUNDLE_SHA256 ?? "unknown",
    staticSetSha256: env.PHASE7_STATIC_SET_SHA256 ?? "unknown",
  };
}
...
export function createPhase7TailnetGateway(config: Phase7TailnetGatewayConfig): Server {
  ...
  const server = createServer((request, response) => {
    ...
    if (url.pathname === "/__phase7r/health") {
    ...
    const failure = phase7TailnetAdmissionFailure({
    ...
    const isTurnStream =
      request.method === "POST" && url.pathname === "/api/phase7/ui/message";
    ...
    upstream.once("error", () => {
      ...
      sendJson(response, 502, {
        error: { code: "upstream_unavailable", message: "Phase 7 preview is unavailable." },
      });
    });
...
export function startPhase7TailnetGateway(env: NodeJS.ProcessEnv = process.env): Server {
  const config = resolvePhase7TailnetGatewayConfig(env);
  const server = createPhase7TailnetGateway(config);
  server.listen(config.listenPort, config.bindHost, () => {
    process.stdout.write(
      `${JSON.stringify({
        event: "phase7_tailnet_gateway_started",
...
if (isMain) {
  const server = startPhase7TailnetGateway();
  const stop = (): void => {

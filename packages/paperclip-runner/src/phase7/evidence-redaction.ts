/**
 * Redaction for Phase 7 live evidence (track 7U).
 *
 ...
 * token metadata, and complete tool results — record classes the Phase 7
 * security boundary forbids on the public surface.
 *
 * The fix is applied at the point evidence is *recorded*, not at the point it
 * is rendered: `Phase7LiveSession` writes only what this module allows, so a
...
export type Phase7EvidenceKind =
  | "session"
  | "provider_event"
  | "tool_exposure"
  | "tool_call"
...
const CATALOG_OPERATION_IDS: ReadonlySet<string> = new Set(
  PHASE7_SEMANTIC_TOOL_CATALOG.map((descriptor) => descriptor.operationId),
);

const CATALOG_INPUT_FIELDS: ReadonlyMap<string, ReadonlySet<string>> = new Map(
  PHASE7_SEMANTIC_TOOL_CATALOG.map((descriptor) => {
 ...
 */
export function phase7AllowlistedInputFields(operationId: string, input: unknown): string[] {
...
function redactToolInput(operationId: string, input: unknown): Record<string, Phase7JsonValue> {
  const summary: Record<string, Phase7JsonValue> = {
    fields: phase7AllowlistedInputFields(operationId, input),
  };
 ...
 */
export function redactPhase7ToolResult(result: unknown): Record<string, Phase7JsonValue> {
  ...
  const entityRefs = Array.isArray(inner.entityRefs)
    ...
    : [];
  const redacted: Record<string, Phase7JsonValue> = {
    ok: record.ok === true,
 ...
 */
export function redactPhase7EvidenceData(
  ...
  switch (kind) {
    ...
    case "tool_result": {
      ...
      return {
        ...
        result: redactPhase7ToolResult(data.result),
      };
    }
    case "interaction":
 ...
 */
export function phase7EvidenceDetail(
  kind: Phase7EvidenceKind,

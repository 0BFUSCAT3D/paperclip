...
export function phase7ScenarioPlan(
...
function derivedPlan(
  ...
  if (work.length === 0) {
    work.push(
      write(
        "report_progress",
        { body: `Phase 7 scenario ${entry.id}: durable progress recorded.` },
        "progress",
 ...
 */
function availableTo(entry: Phase7ScenarioIndexEntry, step: Phase7PlanStep): boolean {
...
function derivedWorkSteps(
  ...
  const marker = `Phase 7 scenario ${entry.id}`;
  ...
  const escapeHatch: Phase7PlanStep[] = entry.scenarioClaims.includes("test:generic_api")
    ? [
        read(
          "generic_api_request",
...
function message(text: string): Phase7PlanStep {
  return { kind: "agent_message", text };
}

function read(operationId: string, input: Phase7JsonValue, summary: string): Phase7PlanStep {

...
import type { Phase7SemanticScenarioPolicy } from "../semantic-tools/types.js";
import { phase7FixtureRunCapabilities } from "./fixture-run-capabilities.js";
import type { CreatePhase7LiveSessionInput } from "./live-session.js";

/**
 * Clean-room chat seed and policy (Phase 7 plan revision 5, track 7M).
 *
...
export const PHASE7_CLEAN_ROOM_SCENARIO_ID = "phase7-clean-room" as const;
 ...
 */
export const PHASE7_CLEAN_ROOM_CLAIMS: readonly string[] = Object.freeze([
...
export const PHASE7_CLEAN_ROOM_WITHHELD_CLAIMS: readonly string[] = Object.freeze([
  "governance:approvals:decide",
  "test:generic_api_request",
  "workspace:control",
]);

export const PHASE7_CLEAN_ROOM_SCENARIO: Phase7SemanticScenarioPolicy = Object.freeze({
  id: PHASE7_CLEAN_ROOM_SCENARIO_ID,
  claims: PHASE7_CLEAN_ROOM_CLAIMS,
...
export interface Phase7CleanRoomIdentityInput {
  token?: string;
  sequence?: number;
}

export function createPhase7CleanRoomIdentity(
  input: Phase7CleanRoomIdentityInput = {},
): Phase7CleanRoomIdentity {
...
export interface Phase7CleanRoomSeedOptions {
  epochMs?: number;
}
 ...
 */
export function createPhase7CleanRoomSeed(
  identity: Phase7CleanRoomIdentity,
  options: Phase7CleanRoomSeedOptions = {},
): Phase7FixtureState {
  return createPhase7FixtureState({
...
const SEEDED_RECORD_KEYS = [
  ...
  "wakes",
  "faults",
] as const satisfies ReadonlyArray<keyof Phase7FixtureState>;
 ...
 */
export function assertPhase7CleanRoomSeedIsBlank(state: Phase7FixtureState): void {
  const seeded = SEEDED_RECORD_KEYS.filter((key) => (state[key] as unknown[]).length > 0);
  if (seeded.length > 0) {
    throw new Error(`Phase 7 clean-room seed must be blank; seeded: ${seeded.join(", ")}`);
  }
  if (state.tasks.length !== 1 || state.actors.length !== 1) {
    throw new Error("Phase 7 clean-room seed must hold exactly one actor and one task");
  }
}

export interface Phase7CleanRoomSessionOptions {
...
export interface Phase7CleanRoomSessionInput {
  identity: Phase7CleanRoomIdentity;
  input: CreatePhase7LiveSessionInput;
}

/**
 * Builds the `Phase7LiveSessionService.create` input for one clean room. The
 ...
 * tool catalog.
 */
export function createPhase7CleanRoomSessionInput(
  options: Phase7CleanRoomSessionOptions,
): Phase7CleanRoomSessionInput {
  const identity = options.identity ?? createPhase7CleanRoomIdentity();
  const seed = createPhase7CleanRoomSeed(identity, {
    ...(options.epochMs === undefined ? {} : { epochMs: options.epochMs }),
  });
  assertPhase7CleanRoomSeedIsBlank(seed);
  return {
    identity,
    input: {
      seed,
      workingDirectory: options.workingDirectory,
      scenario: PHASE7_CLEAN_ROOM_SCENARIO,
      capabilities: phase7FixtureRunCapabilities(PHASE7_CLEAN_ROOM_CLAIMS),
      explicitClaims: [...PHASE7_CLEAN_ROOM_CLAIMS],
      companyId: identity.companyId,

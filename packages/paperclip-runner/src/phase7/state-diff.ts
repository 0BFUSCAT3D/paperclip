...
import type {
  Phase7DiffDomain,
  Phase7DiffDomainSection,
  Phase7DiffFieldChange,
  Phase7DiffRow,
  Phase7StateDiff,
 ...
 * entity domains the Phase 7 plan names. The browser renders this verbatim; it
...
const DOMAINS: Array<{
  ...
  select: (state: Phase7FixtureState) => Entity[];
  noun: string;
}> = [
...
export function phase7StateDiff(
...
function domainSection(
  ...
  return {
    ...
    rows,
    afterSnapshot: JSON.parse(JSON.stringify(after)) as Phase7JsonValue,
  };
}

function describe(entity: Entity): Phase7DiffFieldChange[] {
...
function fieldChanges(before: Entity, after: Entity): Phase7DiffFieldChange[] {

...
const OPEN = {
  ...
  sourceInstanceId: "phase7-tools-test",
};
...
async function runtimeFor(options: {
  ...
  policy?: ConstructorParameters<typeof Phase7SemanticToolRuntime>[0]["policy"];
  ...
  return {
    adapter,
    runtime: new Phase7SemanticToolRuntime({
      adapter,
...
describe("Phase 7 semantic tool catalog", () => {
  it("defines a unique versioned provider-neutral catalog and every optional group", () => {
    const operationIds = PHASE7_SEMANTIC_TOOL_CATALOG.map((tool) => tool.operationId);
    expect(new Set(operationIds).size).toBe(operationIds.length);
    expect(PHASE7_SEMANTIC_TOOL_CATALOG.every((tool) => tool.version === 1)).toBe(true);
    expect(PHASE7_SEMANTIC_TOOL_CATALOG.every((tool) => tool.inputSchema.type === "object")).toBe(true);
    expect(PHASE7_SEMANTIC_TOOL_CATALOG.every((tool) => tool.outputSchema.type === "object")).toBe(true);
    expect(PHASE7_SEMANTIC_TOOL_CATALOG.every((tool) => tool.mockCommandMapping !== undefined)).toBe(true);

    expect(Object.keys(PHASE7_OPTIONAL_TOOL_CATALOGS).sort()).toEqual([
      "cases",
      "company_skills",
    ...
    expect(Object.values(PHASE7_OPTIONAL_TOOL_CATALOGS).every((tools) => tools.length > 0)).toBe(true);
    expect([
      "get_task_context",
      "get_task_history",
      "list_documents",
    ...
    ].every((id) => phase7SemanticTool(id)?.disposition === "always_agent_tool")).toBe(true);
  });
  ...
  it("keeps control-plane operations absent and binds identical fake-agent and Codex surfaces", async () => {
    ...
    const fake = new Phase7FakeAgentToolBinding().bind(visible);
    const codex = new Phase7CodexToolBinding().bind(visible);
    ...
    for (const operationId of PHASE7_CONTROL_PLANE_OWNED_OPERATION_IDS) {
...
describe("Phase 7 exposure and authorization", () => {
  ...
  it("keeps always tools active-task scoped and returns typed denials without protected state", async () => {
    ...
    const approval: Phase7FixtureApproval = {
  ...
  it("emits authorization records with decision reasons and resulting state changes", async () => {
    ...
    expect(validateJsonSchema(
      phase7SemanticTool("report_progress")!.outputSchema,
      structuredClone(result) as unknown as Phase7JsonValue,
    )).toEqual([]);
  });

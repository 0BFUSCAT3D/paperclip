import type {
  Phase7JsonSchema,
  Phase7OptionalCatalogGroup,
  Phase7SemanticToolDescriptor,
  Phase7ToolTaskMode,
} from "./phase7-semantic-tool-types.js";

const ALL_MODES: Phase7ToolTaskMode[] = ["standard", "ask", "planning", "skill_test"];
const WORK_MODES: Phase7ToolTaskMode[] = ["standard", "planning", "skill_test"];
const STANDARD_MODES: Phase7ToolTaskMode[] = ["standard", "skill_test"];
const RESULT_SCHEMA = objectSchema({
  schema: stringSchema({ enum: ["paperclip.phase7.tool-result.v1"] }),
  ok: { type: "boolean" },
...
function stringSchema(options: Partial<Phase7JsonSchema> = {}): Phase7JsonSchema {
  return { type: "string", ...options };
}

function arraySchema(items: Phase7JsonSchema): Phase7JsonSchema {
...
function descriptor(
  value: Omit<Phase7SemanticToolDescriptor, "version" | "redaction"> & {
    redaction?: Phase7SemanticToolDescriptor["redaction"];
  },
): Phase7SemanticToolDescriptor {
...
const ALWAYS_TOOLS: Phase7SemanticToolDescriptor[] = [
  descriptor({
...
function optional(
  operationId: string,
  group: Phase7OptionalCatalogGroup,
  requiredClaims: string[],
  sideEffectClass: Phase7SemanticToolDescriptor["sideEffectClass"],
  mapping: Phase7SemanticToolDescriptor["mockCommandMapping"],
  inputSchema: Phase7JsonSchema = objectSchema({}),
  options: Partial<Pick<Phase7SemanticToolDescriptor, "allowedRoles" | "redaction" | "taskModes" | "idempotency">> = {},
): Phase7SemanticToolDescriptor {
  const title = operationId.split("_").map((part) => part[0]!.toUpperCase() + part.slice(1)).join(" ");
  return descriptor({
    operationId,
    title,
    description: `${title} through the Phase 7 ${group.replaceAll("_", " ")} capability set.`,
    inputSchema,
    outputSchema: RESULT_SCHEMA,
...
const OPTIONAL_TOOLS: Phase7SemanticToolDescriptor[] = [
...
export const PHASE7_SEMANTIC_TOOL_CATALOG: readonly Phase7SemanticToolDescriptor[] = Object.freeze(
  [...ALWAYS_TOOLS, ...OPTIONAL_TOOLS].map((tool) => Object.freeze(tool)),
);

export const PHASE7_OPTIONAL_TOOL_CATALOGS: Readonly<
  Record<Phase7OptionalCatalogGroup, readonly Phase7SemanticToolDescriptor[]>
> = Object.freeze({
...
export function phase7SemanticTool(operationId: string): Phase7SemanticToolDescriptor | undefined {
  return PHASE7_SEMANTIC_TOOL_CATALOG.find((tool) => tool.operationId === operationId);
}

function toolsInGroup(group: Phase7OptionalCatalogGroup): readonly Phase7SemanticToolDescriptor[] {

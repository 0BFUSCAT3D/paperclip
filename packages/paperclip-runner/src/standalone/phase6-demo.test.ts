...
import {
  createLegacyPhase6StandaloneAdapter,
  createNativePhase6StandaloneAdapter,
  resolvePhase6StandaloneMode,
  runPhase6StandaloneDemo,
  type Phase6StandaloneAdapter,
} from "./phase6-demo.js";

function adapterSpy(adapter: Phase6StandaloneAdapter) {
  return {
    adapter,
...
describe("Phase 6 standalone demo", () => {
  it("keeps the legacy path as the default", async () => {
    const legacy = adapterSpy(createLegacyPhase6StandaloneAdapter());
    const createLegacyAdapter = vi.fn(() => legacy.adapter);
    const createNativeAdapter = vi.fn(() => createNativePhase6StandaloneAdapter());
    const result = await runPhase6StandaloneDemo({
  ...
  it("runs the native demo behind the feature flag", async () => {
    const native = adapterSpy(createNativePhase6StandaloneAdapter());
    const createNativeAdapter = vi.fn(() => native.adapter);
    const createLegacyAdapter = vi.fn(() => createLegacyPhase6StandaloneAdapter());
    const result = await runPhase6StandaloneDemo({
  ...
  it("constructs and invokes only legacy when the kill switch is enabled", async () => {
    expect(resolvePhase6StandaloneMode({
      featureFlagEnabled: true,

...
2. Create a Paperclip `request_confirmation` interaction targeted at that issue
   ...
   continuation policy that wakes the assignee after acceptance or rejection.
3. Put the issue in `in_review` and stop. Do not prepare App worktrees or code
   while the interaction is pending.
4. On rejection, revise Phase A only and present a new revision. If the research
   PR head, gate document, or connection set changes, withdraw/supersede the old
   confirmation and request a fresh one.

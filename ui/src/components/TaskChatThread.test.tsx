...
/**
 * TaskChatThread pulls in live-transcript polling, the sidebar/mobile context,
 * the plan-document query, and the rich MarkdownEditor. For a draft
 * pass-through test we only need the composer to mount, so each heavy
 * dependency is stubbed and the editor is reduced to a node that echoes its
 * `value` — which is the restored draft once TaskChatComposer wires draftKey in.
 */
...
describe("TaskChatThread draft pass-through", () => {
  it("forwards draftKey to the composer so a stored draft restores on mount", () => {
    ...
    expect(editor?.textContent).toBe("half-written thought");
  });
});

...
- execution workspaces, project workspaces, workspace runtime services, and workspace operations. Workspace reads
  ...
  A code-fix completion claim must register an inspectable branch, commit, or pull-request work product appropriate to
  the requested handoff and prove that the cited change is on that delivery path. Managed runtime command postconditions,
  bounded isolated-workspace port allocation, desired-versus-actual state, and delivery-evidence semantics are defined
  under adapter-backed workspace coherence in `doc/execution-semantics.md`.
